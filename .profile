...
# spit out the definition of a function or alias
# usage: define <function or alias name> [...function|alias] ...
# Useful to do things like exporting definitions to sudo context
# or seeing if one definition is masking another
define() {
  local word="$1"
  shift
  if [ -z "$word" ] && [ -z "$1" ]; then
    echo "Usage: define <function or alias name> [...function|alias] ..."
    return 1
  fi
    ...
    for type in $(type -a -t $word | uniq); do
      case $type in
        ...
        file)
          ...
          ;;
        *)
          note "$word is not a builtin, function, alias, or file; it is a $type"
          # return 1
          ;;
      esac
    done
