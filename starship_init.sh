...
starship_preexec() {
    ...
    if [ "${STARSHIP_PREEXEC_READY:-}" = "true" ]; then
        ...
        STARSHIP_START_TIME=$(/run/current-system/sw/bin/starship time)
    fi
...
starship_precmd() {
    ...
    if [[ -n "${STARSHIP_START_TIME-}" ]]; then
        STARSHIP_END_TIME=$(/run/current-system/sw/bin/starship time)
        STARSHIP_DURATION=$((STARSHIP_END_TIME - STARSHIP_START_TIME))
        ARGS+=( --cmd-duration="${STARSHIP_DURATION}")
        STARSHIP_START_TIME=""
    fi
    PS1="$(/run/current-system/sw/bin/starship prompt "${ARGS[@]}")"
    if [[ ${BLE_ATTACHED-} ]]; then
        local nlns=${PS1//[!$'\n']}
        bleopt prompt_rps1="$nlns$(/run/current-system/sw/bin/starship prompt --right "${ARGS[@]}")"
    fi
...
# then hook our functions into their framework.
    ...
    if [[ -n "${BASH_VERSION-}" ]] && [[ "${BASH_VERSINFO[0]}" -gt 4 || ( "${BASH_VERSINFO[0]}" -eq 4 && "${BASH_VERSINFO[1]}" -ge 4 ) ]]; then
        starship_preexec_ps0() {
            /run/current-system/sw/bin/starship time
        }
...
PS2="$(/run/current-system/sw/bin/starship prompt --continuation)"
