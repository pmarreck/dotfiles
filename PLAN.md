...
- [x] Run the dotfiles suite after removal, retain recoverability, and commit
  ...
  - Reconciled the divergent local and remote histories without rewriting
    either. Combined shell-startup assertions retain Node/npm and rehash
    checks. All 186 merged host test files and the sandboxed check passed.
    Run the pre-push hook under `nix develop` to use declared shell dependencies.
