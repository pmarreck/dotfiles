...
#
# Peter van Eerten - March 2009/June 2011. License: GPL version 3.
#
#---------------------------------------------------------------------------------------------------------------------
# CREDITS:
...
# Version of BACON
declare -rx g_VERSION="1.0 build 23"
...
function Handle_Open
{
	...
	@(MEMORY) )
	    ...
	    echo "if(!__b2c__trap){if(__b2c__memory__check($HANDLE)) {ERROR=1; if(!__b2c__catch_set){fprintf(stderr, \"%s\n\", ERR${g_STRINGSIGN}(ERROR)); exit(ERROR);} if(!setjmp(__b2c__jump)) goto $g_CATCHGOTO;} }" >> $g_CFILE;;
	@(NETWORK) )
	    # Network code
...
function Handle_Getbyte
{
    ...
    # Translate function to C function
    echo "if(!__b2c__trap){if(__b2c__memory__check((char *)$POS)) {ERROR=1; if(!__b2c__catch_set){fprintf(stderr, \"%s\n\", ERR${g_STRINGSIGN}(ERROR)); exit(ERROR);} if(!setjmp(__b2c__jump)) goto $g_CATCHGOTO;} }" >> $g_CFILE
...
function Handle_Read
{
	...
	then
	    ...
	    CHECK=${CHECK##*:}
	    echo "${1} = (${CHECK%% *})__b2c__floatarray[__b2c__floatarray_ptr];" >> $g_CFILE
...
function Handle_Return
{
   ...
   # Check type of var, func using string but returning a value?
	...
	g_FUNCTYPE="double "
	ARG=${1#*\(}
	echo "__b2c__rbuffer_ptr++; if(__b2c__rbuffer_ptr >= $g_MAX_RBUFFERS) __b2c__rbuffer_ptr=0; if(${ARG%\)*} != NULL)" >> $g_CFILE
	echo " {__b2c__rbuffer[__b2c__rbuffer_ptr] = (char*)realloc(__b2c__rbuffer[__b2c__rbuffer_ptr], (strlen(${ARG%\)*})+1)*sizeof(char));" >> $g_CFILE
	echo "strcpy(__b2c__rbuffer[__b2c__rbuffer_ptr], ${ARG%\)*} );}" >> $g_CFILE
...
function Parse_Line
{
	...
	case "${STATEMENT}" in
		...
		then
		    ...
		    while read -r LINE
		    do
			echo -e -n "\rConverting '${INC}'... $g_COUNTER"
		...
		then
		    ...
		    SYM=${1#* }
		    echo "if (!__b2c__trap){if(__b2c__memory__check((char*)${SYM%%,*})) {ERROR=1; fprintf(stderr, \"%s\n\", ERR${g_STRINGSIGN}(ERROR)); exit(ERROR);} }" >> $g_CFILE
		...
		then
			...
			echo "if(!__b2c__trap){if(__b2c__memory__check((char *)${1#* }))" >> $g_CFILE
			echo "{ERROR=1; if(!__b2c__catch_set){fprintf(stderr, \"%s\n\", ERR${g_STRINGSIGN}(ERROR)); exit(ERROR);} if(!setjmp(__b2c__jump)) goto $g_CATCHGOTO;} } free((void*)${1#* });" >> $g_CFILE
		    fi
		fi;;
	    "GOTO")
		# Check argument
		...
		then
		    ...
		    # Translate to C label
		    echo "__b2c__gosub_buffer_ptr++; if (__b2c__gosub_buffer_ptr >= $g_MAX_RBUFFERS) {ERROR=31; if(!__b2c__catch_set){fprintf(stderr, \"%s\n\", ERR${g_STRINGSIGN}(ERROR)); exit(ERROR);} if(!setjmp(__b2c__jump)) goto $g_CATCHGOTO;}" >> $g_CFILE
		...
		then
		    ...
		    then
			INC=${EXP#*\(}
			echo "if(__b2c__${EXP%%\(*}_exist(${INC} == NULL) __b2c__${EXP%%\(*}__add(${INC};" >> $g_CFILE
			echo "__b2c__${EXP%%\(*}_exist(${INC}->value = ${EXP} + ${SYM};" >> $g_CFILE
			# Check for relations
...
echo "#define SQR(__b2c__x) sqrt((double)__b2c__x)" >> $g_HFILE
echo "#define POW(__b2c__x, __b2c__y) pow((double)__b2c__x, (double)__b2c__y)" >> $g_HFILE
...
echo "#define EVEN(__b2c__x) (((long)(__b2c__x) % 2 == 0) ? 1 : 0)" >> $g_HFILE
echo "#define ODD(__b2c__x) (((long)(__b2c__x) % 2 != 0) ? 1 : 0)" >> $g_HFILE
...
echo "int __b2c__sbuffer_ptr = 0;" >> $g_HFILE
echo "char* __b2c__rbuffer[$g_MAX_RBUFFERS] = { NULL };" >> $g_HFILE
echo "int __b2c__rbuffer_ptr = 0;" >> $g_HFILE
echo "static jmp_buf __b2c__gosub_buffer[$g_MAX_RBUFFERS] = { 0 };" >> $g_HFILE
echo "int __b2c__gosub_buffer_ptr = -1;" >> $g_HFILE
...
echo "int __b2c__memory__check(void* __b2c__x) {if (stat(__b2c__x, &__b2c__sb) == -1 && errno == EFAULT) return(1); else return(0);}" >> $g_HFILE
...
echo "void *__b2c__realloc(void* __b2c__x, long __b2c__y) {if(__b2c__x == NULL) return (NULL); if(!__b2c__trap) {if(__b2c__memory__check((char*)__b2c__x))" >> $g_HFILE
echo "{ERROR=1;fprintf(stderr, \"%s\n\", ERR${g_STRINGSIGN}(ERROR)); exit(ERROR);} } switch(__b2c__memtype) {case 1: __b2c__x = realloc((void*)__b2c__x, sizeof(char)*(__b2c__y+1)); break;" >> $g_HFILE
...
echo "char __b2c__peek(char* __b2c__x) {if(!__b2c__trap) {if(__b2c__memory__check((char*) __b2c__x)) {ERROR=1; fprintf(stderr, \"%s\n\", ERR${g_STRINGSIGN}(ERROR)); exit(ERROR);} } return((char)*__b2c__x);}" >> $g_HFILE
echo "short __b2c__peeks(short* __b2c__x) {if(!__b2c__trap) {if(__b2c__memory__check((short*) __b2c__x)) {ERROR=1; fprintf(stderr, \"%s\n\", ERR${g_STRINGSIGN}(ERROR)); exit(ERROR);} } return((short)*__b2c__x);}" >> $g_HFILE
echo "int __b2c__peeki(int* __b2c__x) {if(!__b2c__trap) {if(__b2c__memory__check((int*) __b2c__x)) {ERROR=1; fprintf(stderr, \"%s\n\", ERR${g_STRINGSIGN}(ERROR)); exit(ERROR);} } return((int)*__b2c__x);}" >> $g_HFILE
echo "long __b2c__peekl(long* __b2c__x) {if(!__b2c__trap) {if(__b2c__memory__check((long*) __b2c__x)) {ERROR=1; fprintf(stderr, \"%s\n\", ERR${g_STRINGSIGN}(ERROR)); exit(ERROR);} } return((long)*__b2c__x);}" >> $g_HFILE
echo "float __b2c__peekf(float* __b2c__x) {if(!__b2c__trap) {if(__b2c__memory__check((float*) __b2c__x)) {ERROR=1; fprintf(stderr, \"%s\n\", ERR${g_STRINGSIGN}(ERROR)); exit(ERROR);} } return((float)*__b2c__x);}" >> $g_HFILE
echo "double __b2c__peekd(double* __b2c__x) {if(!__b2c__trap) {if(__b2c__memory__check((double*) __b2c__x)) {ERROR=1; fprintf(stderr, \"%s\n\", ERR${g_STRINGSIGN}(ERROR)); exit(ERROR);} } return((double)*__b2c__x);}" >> $g_HFILE
echo "char __b2c__peekv(char* __b2c__x) {if(!__b2c__trap) {if(__b2c__memory__check((char*) __b2c__x)) {ERROR=1; fprintf(stderr, \"%s\n\", ERR${g_STRINGSIGN}(ERROR)); exit(ERROR);} } return((char)*__b2c__x);}" >> $g_HFILE
...
echo "case 31: strcpy(__b2c__warn, \"Gosub nesting too deep!\"); break;" >> $g_HFILE
...
while read -r LINE
do
    echo -e -n "\rConverting '${FEED}'... $g_COUNTER   "
